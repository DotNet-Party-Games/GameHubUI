...
export class BlackjackComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.bjdataSub=this.socketService.getBlackJackData().subscribe(data => {
      ...
      this.audioSub= this.socketService.getAudioTrigger().subscribe(data2 => {
        this.playSFX(data2);
      })
    });
  }
